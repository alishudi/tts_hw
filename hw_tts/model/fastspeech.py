...
class FastSpeech2(BaseModel):
    ...
    def forward(self, src_seq, src_pos, mel_pos=None, mel_max_len=None, duration=None, energy=None, alpha=1.0, **batch):
        ...
        if self.training:
            ...
            energy_embedding, energy_predictor_output = self.energy_predictor(output, alpha, energy)
            ...
            energy_embedding, _ = self.energy_predictor(output, alpha)
            output = self.decoder(output, mel_pos, energy_embedding)
            output = self.mel_linear(output)
            return output
